...
const MyBookings = () => {
  ...
  const fetchMyBookings = useCallback(async () => {
    try {
      const url = user?.email
        ? `https://hariye-tour-agency-u7bh.onrender.com/api/readBooking?email=${encodeURIComponent(user.email)}`
  ...
  const removeBooking = async (id) => {
    if (window.confirm("Are you sure you want to cancel this booking?")) {
      try {
        await axios.delete(
          `https://hariye-tour-agency-u7bh.onrender.com/api/deleteBooking/${id}`,
        );
            ...
            bookings.map((item) => (
              ...
              >
                <div className="w-full md:w-[280px] h-[200px] overflow-hidden">
                  <img
                    src={`https://hariye-tour-agency-u7bh.onrender.com/images/${item.tourId?.image}`}
